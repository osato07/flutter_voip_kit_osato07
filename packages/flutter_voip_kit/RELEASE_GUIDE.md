...
`packages/flutter_voip_kit` フォルダがリポジトリの中階層にある状態です。

### パターンB：ライブラリ単体として切り出す場合（推奨）
よりきれいに管理したい場合、新しいリポジトリを作成し、`packages/flutter_voip_kit` の中身だけを移動させます。
...
dependencies:
  flutter_voip_kit:
    git:
      ...
      path: packages/flutter_voip_kit  # パッケージがあるフォルダパス
...
dependencies:
  flutter_voip_kit:
    git:
      url: https://github.com/あなたのユーザー名/flutter_voip_kit.git
      ref: main
```
...
### 手順
1.  **ドキュメント生成**:
    ターミナルで `packages/flutter_voip_kit` フォルダに移動し、以下を実行します。
    ```bash
    dart doc .
    ```
